...
public class ThuocDAO extends InterfaceDAO<Thuoc, String> {
	...
	private final String INSERT_SQL = "INSERT INTO Thuoc "
			+ "(idThuoc, tenThuoc, hinhAnh, thanhPhan, idDVT, idDM, idXX, soLuongTon, giaNhap, donGia, hanSuDung) "
			+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	private final String UPDATE_SQL = "UPDATE Thuoc SET tenThuoc = ?, hinhAnh = ?, thanhPhan = ?, idDVT = ?, idDM = ?, idXX = ?,soLuongTon = ?, giaNhap = ?, donGia = ?, hanSuDung = ? "
			+ "WHERE idThuoc = ?";

	private final String DELETE_BY_ID = "DELETE from Thuoc where idThuoc = ?";

	private final String SELECT_ALL_SQL = "SELECT Thuoc.*, " + "DonViTinh.idDVT, DonViTinh.ten AS tenDVT, "
			+ "DanhMuc.idDM, DanhMuc.ten AS tenDM, " + "XuatXu.idXX, XuatXu.ten AS tenXX " + "FROM Thuoc "
			+ "INNER JOIN DonViTinh ON Thuoc.idDVT = DonViTinh.idDVT "
			+ "INNER JOIN DanhMuc ON Thuoc.idDM = DanhMuc.idDM " + "INNER JOIN XuatXu ON Thuoc.idXX = XuatXu.idXX";

	private final String SELECT_BY_ID = "SELECT Thuoc.*, " + "DonViTinh.idDVT, DonViTinh.ten AS tenDVT, "
			+ "DanhMuc.idDM, DanhMuc.ten AS tenDM, " + "XuatXu.idXX, XuatXu.ten AS tenXX " + "FROM Thuoc "
			+ "INNER JOIN DonViTinh ON Thuoc.idDVT = DonViTinh.idDVT "
			+ "INNER JOIN DanhMuc ON Thuoc.idDM = DanhMuc.idDM " + "INNER JOIN XuatXu ON Thuoc.idXX = XuatXu.idXX "
			+ "WHERE Thuoc.idThuoc = ?";
	...
	public void create(Thuoc e) {
		JDBCConnection.update(INSERT_SQL, e.getId(), e.getTenThuoc(), e.getHinhAnh(), e.getThanhPhan(),
				e.getDonViTinh().getId(), e.getDanhMuc().getId(), e.getXuatXu().getId(), e.getSoLuongTon(),
				e.getGiaNhap(), e.getDonGia(), e.getHanSuDung());
	}
